...
export default function ReportPreview() {
  ...
  useEffect(() => {
    ...
    void fetchReport(selectedReportId, controller.signal)
      .then((nextReport) => {
    ...
    <main className="min-h-screen bg-slate-100 px-4 py-12 text-slate-950">
      <section className="mx-auto max-w-2xl rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <p className="mt-2 text-sm text-slate-600">
          Select the slow quarterly report, then quickly switch to another
          report. Request events are logged in the browser console.
        </p>
        ...
        <div
          className="mt-6 min-h-44 rounded-2xl border border-slate-200 bg-slate-50 p-5"
          aria-live="polite"
        >
          {isLoading ? (

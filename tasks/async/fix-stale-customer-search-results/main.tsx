...
async function searchCustomers(
  ...
  const delay =
    normalizedQuery === "error" || normalizedQuery.length <= 2 ? 900 : 250;
...
export default function CustomerSearch() {
  ...
  function handleQueryChange(event: ChangeEvent<HTMLInputElement>) {
    ...
    void searchCustomers(normalizedQuery, controller.signal)
      .then((customers) => {
    ...
    <main className="min-h-screen bg-zinc-100 px-4 py-12 text-zinc-950">
      <section className="mx-auto max-w-xl rounded-3xl border border-zinc-200 bg-white p-6 shadow-sm">
        ...
        <p className="mt-2 text-sm text-zinc-600">
          Try typing <code className="rounded bg-zinc-100 px-1">al</code> and
          then <code className="rounded bg-zinc-100 px-1">ali</code> quickly.
          Use <code className="rounded bg-zinc-100 px-1">error</code> to
          simulate a failure.
        </p>

...
export default function SearchableSelectionList() {
  ...
  const selectedCustomerIdsSet = useMemo(
  ...
  const visibleCustomers = useMemo(
    () =>
      CUSTOMERS.filter((customer) => {
        ...
        const matchesSelection =
          !showSelectedOnly || selectedCustomerIdsSet.has(customer.id);
    ...
    [normalizedQuery, showSelectedOnly, selectedCustomerIdsSet],
  );

  const handleToggle = useCallback((customerId: string) => {
    setSelectedCustomerIds((currentIds) => {
      if (currentIds.includes(customerId)) {
        return currentIds.filter((id) => id !== customerId);
      }

      return [...currentIds, customerId];
    });
  }, []);
    ...
    <main className="mx-auto max-w-3xl p-6">
      ...
      <ul className="mt-3 max-h-128 overflow-y-auto rounded border border-slate-300 bg-white">
        {visibleCustomers.map((customer) => (
          <CustomerRow
            customer={customer}
            isSelected={selectedCustomerIdsSet.has(customer.id)}
            key={customer.id}

...
export default function ChannelActivity() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = subscribeToChannel(selectedChannelId, (event) => {
    ...
    <main className="min-h-screen bg-zinc-100 px-4 py-12 text-zinc-950">
      <section className="mx-auto max-w-2xl rounded-3xl border border-zinc-200 bg-white p-6 shadow-sm">
        ...
        <p className="mt-2 text-sm text-zinc-600">
          Wait for an event, then switch channels. Subscription events are
        ...
        <div
          className="mt-6 min-h-72 rounded-2xl border border-zinc-200 bg-zinc-50 p-5"
          aria-live="polite"
        >
                  ...
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs font-medium text-zinc-500">
                        {sourceChannel?.name ?? event.channelId} ·{" "}
                        {event.receivedAt}
                      </span>
                    </div>
